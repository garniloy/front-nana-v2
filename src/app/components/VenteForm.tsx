...
// ─── PATCH 1 : import QuickRegisterModal ─────────────────────────────────────
import QuickRegisterModal from './Quickregistermodal';

// ─── Types ────────────────────────────────────────────────────────────────────
// ─── PATCH 2 : ProdServ type unchanged — fields are the same in ref_prod_serv ─
type ProdServ = {
  type: 'prod' | 'serv';
  nom: string;
  pr_stock: number;
...
// ─── PATCH 3 : QuickRegisterModal state type ──────────────────────────────────
...
// ─── Sub-components ───────────────────────────────────────────────────────────
// ─── PATCH 4 : SuggestionPopup — add "＋ Ajouter" shortcut buttons ────────────
...
export default function Vente({ onclose }: onCloseProps) {
  ...
  // ─── PATCH 5 : QuickRegisterModal state ──────────────────────────────────────
  ...
  // ── Fetch data ──
  // ─── PATCH 6 : fetch from ref_prod_serv filtered by office instead of prod_serv ─
  const fetchAll = async (office: string) => {
    ...
    try {
      const [stockRes, prodServRes, sellersRes, clientsRes] = await Promise.all([
        ...
        // ← was: prod_serv with no office filter
        // ← now: ref_prod_serv filtered by office + is_deleted: false
        getDataFromTableWithConstraints('ref_prod_serv', {
        ...
        getDataFromTableWithConstraints('seller', {
          ...
          constraints: { is_deleted: false, owner: user.promoted_by },
        }),
        getDataFromTableWithConstraints('client', {
          fields: ['id', 'name', 'sexe', 'phone', 'seller'],
          constraints: { owner: user.promoted_by },
        }),
      ]);
  ...
  // ── Re-run recomputeAll whenever clientKind or seller changes ──
  useEffect(() => {
    if (sell.details.alements.length === 0) return;
    if (!sell.seller) return;
    setSell((d: SellState) => {
  ...
  // ─── PATCH 7 : helpers to decide whether to show "add new" shortcut ───────────
  // Show "＋ Ajouter" in seller popup when query ≥ 2 chars and no exact match
  ...
  // Show "＋ Ajouter" in client popup when query ≥ 2 chars and no exact match in either list
  ...
  // ─── PATCH 8 : QuickRegisterModal handlers ────────────────────────────────────
  const currentOffice = selectedOffice ?? user?.office ?? '';

  function openQuickClient() {
    setShowClientSugg(false);
    setQuickRegister({ open: true, type: 'client', prefillName: clientInput });
  }

  function openQuickDistributor() {
    setShowSellerSugg(false);
    setQuickRegister({ open: true, type: 'distributor', prefillName: sellerInput });
  }

  // Called when QuickRegisterModal succeeds — re-fetch then auto-select the new person
  async function handleQuickRegisterSuccess(name: string) {
    setQuickRegister((prev) => ({ ...prev, open: false }));
    // re-fetch sellers + clients for this office
    ...
    // auto-select the newly registered person in the client field
    if (quickRegister.type === 'client') {
      setClientInput(name);
      // selectClient will run after state settles via the updated clients list
      setTimeout(() => {
        const asClient = (clientsRes.list ?? []).find(
          (c: Client) => c.name.toLowerCase() === name.toLowerCase(),
        );
        if (asClient) {
          const clientSeller = (sellersRes.list ?? []).find(
            (s: Seller) => s.id === asClient.seller,
          );
          setClientInput(asClient.name);
          setSell((d: SellState) => {
            d.clientObj          = { ...asClient, _kind: 'client' } as typeof d.clientObj;
      ...
      // distributor: auto-select in client field (distributor path)
      setClientInput(name);
      setTimeout(() => {
        const asSeller = (sellersRes.list ?? []).find(
          (s: Seller) => s.name.toLowerCase() === name.toLowerCase(),
        );
        if (asSeller) {
          const ownerForSale = (sellersRes.list ?? []).find(
            (s: Seller) => s.id === String(user?.promoted_by ?? ''),
          );
          setClientInput(asSeller.name);
          setSell((d: SellState) => {
            d.clientObj          = { ...asSeller, _kind: 'distributor' } as typeof d.clientObj;
            ...
            d.details.clientName = asSeller.name;
            if (ownerForSale) {
              d.sellerObj          = ownerForSale;
              d.seller             = ownerForSale.id;
              d.office             = ownerForSale.office;
              d.details.sellerName = currentOffice;
            }
            recomputeAll(d, 'distributor', ownerForSale?.id ?? d.seller, prodServList);
          });
          if (ownerForSale) { setSellerInput(ownerForSale.name); setSellerLocked(true); }
    ...
    setSell((d: SellState) => {
      ...
      if (existing !== -1) {
        ...
        d.details.alements[existing] = {
          name: ps.nom, type: ps.type, qty: newQty, total: newTotal,
          benef: newBenef, pv: newPv, commission: newCommSlot,
        };
      } else {
        d.details.alements.push({
          name: ps.nom, type: ps.type, qty, total: lineTotal,
  ...
  return (
    <>
      <style>{`
        ...
        .field-block { display: flex; flex-direction: column; gap: 0.5rem; min-height: fit-content; justify-content: flex-start; position: relative; width: 100%; }
        ...
        .input, select.input, textarea.input { width: 100%; min-width: 0; box-sizing: border-box; }
        ...
        .summary-bar { display: flex; flex-wrap: wrap; align-items: center; gap: 1rem; padding: clamp(12px, 2vw, 20px); border-bottom: 1px solid var(--nm-dark); flex-shrink: 0; width: 100%; }
        ...
        .prod-form { display: grid; grid-template-columns: minmax(0, 1fr) minmax(90px, 120px) minmax(110px, 140px) auto; gap: 1rem; align-items: end; padding: clamp(12px, 2vw, 20px); border-bottom: 1px solid var(--nm-dark); flex-shrink: 0; width: 100%; min-width: 0; }
        ...
        .details-panel { flex: 1; overflow-y: auto; overflow-x: hidden; padding: clamp(12px, 2vw, 20px); min-height: 0; min-width: 0; width: 100%; }
        ...
        .detail-item { display: grid; grid-template-columns: minmax(120px, 1fr) 32px auto auto auto minmax(140px, 1fr) auto; align-items: center; gap: 0.75rem; padding: 1rem; min-width: 0; border-radius: var(--radius-xl); background: var(--nm-bg); box-shadow: 5px 5px 10px var(--nm-dark), -5px -5px 10px var(--nm-light); }
        ...
        .di-rowsum { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 0.5rem; min-width: 0; }
        ...
        @media (max-width: 1100px) {
          ...
          .step-side { border-right: none; border-bottom: 1px solid var(--nm-dark); max-height: none; }
          .summary-bar { justify-content: flex-start; }
          .detail-item { grid-template-columns: 1fr; align-items: flex-start; }
          .di-rowsum { justify-content: flex-start; }
        }
        @media (max-width: 768px) {
          .vente-header { flex-wrap: wrap; gap: 1rem; }
          .prod-form { grid-template-columns: 1fr; }
          .qty-group, .price-group { display: flex !important; }
          .summary-bar { flex-direction: column; align-items: flex-start; }
          .sbar-total { margin-left: 0; text-align: left; }
          .sell-summary { flex-direction: column; align-items: stretch; height: auto; padding: 1rem; }
          .s-sep { display: none; }
        }
      `}</style>
              ...
              <div className="fields">
                ...
                <div className="field-block">
                  ...
                  <div className="input-wrap">
                    ...
                    />
                    {/* ─── PATCH 9 : seller suggestion with "＋ Ajouter distributeur" ─── */}
                    {showSellerSugg && !sellerLocked && (sellerSugg.length > 0 || shouldShowAddSeller(sellerInput, sellerSugg)) && (
                      <SuggestionPopup
                        items={sellerSugg}
                        onSelect={selectSeller}
                        onAddNew={shouldShowAddSeller(sellerInput, sellerSugg) ? openQuickDistributor : undefined}
                        addLabel={`Enregistrer "${sellerInput}" comme distributeur`}
                      />
                    )}
                  </div>
                ...
                <div className="field-block">
                  ...
                  <div className="input-wrap">
                    ...
                    />
                    {/* ─── PATCH 10 : client suggestion with "＋ Ajouter client" ──────── */}
                    {showClientSugg && (clientSugg.length > 0 || shouldShowAddClient(clientInput, clientSugg)) && (
                      <SuggestionPopup
                        items={clientSugg}
                        onSelect={selectClient}
                        onAddNew={shouldShowAddClient(clientInput, clientSugg) ? openQuickClient : undefined}
                        addLabel={`Enregistrer "${clientInput}" comme client`}
                      />
                    )}
                  </div>
                  {sell.clientObj && (
      ...
      {/* ─── PATCH 11 : QuickRegisterModal mount ─────────────────────────────── */}
      {quickRegister.open && (
        <QuickRegisterModal
